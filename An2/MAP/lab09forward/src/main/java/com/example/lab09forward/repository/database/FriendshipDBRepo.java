...
public class FriendshipDBRepo extends AbstractDBRepo<UUID, Friendship> {
    ...
    public List<Friendship> loadAllData() {
        ...
        try (Connection connection = DriverManager.getConnection(dbLink, username, password);
            ...
            while (resultSet.next()) {
                ...
                Friendship friendship = new Friendship(id1, id2, date, confirmed, sentDate);
                friendship.setId(id);
    ...
    public Friendship findByUUIDs(UUID id1, UUID id2) {
        try (Connection connection = DriverManager.getConnection(dbLink, username, password);
            ...
            resultSet.next();
            UUID id = UUID.fromString(resultSet.getString("uid"));
            UUID id1_ = UUID.fromString(resultSet.getString("user1ID"));
            UUID id2_ = UUID.fromString(resultSet.getString("user2ID"));
            LocalDate date = resultSet.getDate("date").toLocalDate();
            LocalDate sentDate = resultSet.getDate("sentDate").toLocalDate();
            Boolean confirmed = resultSet.getBoolean("confirmed");
            Friendship friendship = new Friendship(id1_, id2_, date, confirmed, sentDate);
            friendship.setId(id);
            return friendship;
