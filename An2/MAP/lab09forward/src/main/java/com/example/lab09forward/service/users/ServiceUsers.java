package com.example.lab09forward.service.users;
...
public class ServiceUsers {
    ...
    public void buildFriendsList(User loggedUser) {
        Iterable<Friendship> friendships = repoControllerUF.findAllFriendships();
        for(Friendship friendship : friendships){

            if(friendship.getId1().equals(loggedUser.getId()) && friendship.getConfirmed() == Boolean.TRUE){
                loggedUser.addFriend(findUser(friendship.getId2()));
            }
            else if(friendship.getId2().equals(loggedUser.getId()) && friendship.getConfirmed() == Boolean.TRUE){
                loggedUser.addFriend(findUser(friendship.getId1()));
            }
        }
    }

    public boolean areFriends(User friend, User loggedUser) {
        for (User user : loggedUser.getFriends()) {
